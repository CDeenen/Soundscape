...
import {helpMenuSoundboardConfig} from "./helpMenu.js";
...
export class soundboardConfig extends FormApplication {
    ...
    async getData() {
        ...
        this.playlistName = settings.playlistName;
        this.soundName = settings.soundName;
        ...
        for (let p of game.playlists) {
            ...
            if (p.name == settings.playlistName)
                for (let s of p.sounds.contents) 
                    sounds.push({name:s.name,id:s.id});
        }

        let pl, snd;
        if (settings.playlistName != undefined && settings.playlistName != "")                pl = game.playlists.getName(settings.playlistName);
        if (settings.soundName != undefined && settings.soundName != "" && pl != undefined)   snd = pl.sounds.getName(settings.soundName);
